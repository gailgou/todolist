...
def show_todo_list():
    ...
    if request.method == 'GET':
        ...
        completed_todos = [t for t in todolists if t.status == 1]
        uncompleted_todos = [t for t in todolists if t.status == 0]
        
        return render_template('index.html', completed_todos=completed_todos, 
                               uncompleted_todos=uncompleted_todos, form=form, sort_order=sort_order)
    else:
    ...
    if todo_ids:
        for todo_id in todo_ids:
            ...
            if todo:
                todo.status = 1
        db.session.commit()
        flash('Batch complete successfully')
    else:
...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        if not existing_user:
            ...
            # 添加默认的待办事项
            now = int(time.time())
            todo1 = TodoList(user_id=user.id, title='示例待办事项1', status=0)
            todo2 = TodoList(user_id=user.id, title='示例待办事项2', status=1)
